...
function parseAccountKit(text){if(!text.startsWith('FREE-RK1.'))throw new Error(currentLang==='vi'?'Recovery Kit không hợp lệ.':'Invalid Recovery Kit.');return JSON.parse(dec.decode(unb64url(text.slice(9))))}
async function decryptAccountRecoveryKit(text,pin){if(!validPin(pin))throw new Error(currentLang==='vi'?'PIN phải gồm 4–6 chữ số.':'PIN must contain 4–6 digits.');const kit=parseAccountKit(text);const key=await deriveAccountRecoveryKey(unb64url(kit.secret),pin,unb64url(kit.salt));let plain;try{plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:unb64url(kit.iv)},key,unb64url(kit.ciphertext))}catch{throw new Error(currentLang==='vi'?'Recovery Kit hoặc PIN không đúng.':'Recovery Kit or PIN is incorrect.')}const state=JSON.parse(dec.decode(plain));if(state.format!=='FREE-ACCOUNT-1'||!state.identity)throw new Error('Invalid FREE account payload');const expected=await identityId(state.identity.kemPublicKey,state.identity.sigPublicKey);if(expected!==state.identity.id)throw new Error('Recovery identity integrity check failed');return{state,secret:unb64url(kit.secret)}}
async function saveNewAccountKit(pin){const kit=await buildAccountRecoveryKit(pin);await setKV('accountRecoveryKit',kit);$('#accountRecoveryKit').value=kit;return kit}
...
function renderIdentity(){if(!me)return;const link=inviteLink();$('#myId').textContent=me.id;$('#shortId').textContent=me.shortId;$('#displayNameHeading').textContent=profile?.displayName||'FREE';$('#profileName').value=profile?.displayName||'';$('#inviteLink').value=link;$('#qr').src=`/api/qr?text=${encodeURIComponent(link)}`;const kit=$('#accountRecoveryKit');if(kit)getKV('accountRecoveryKit').then(v=>{if(v)kit.value=v})}
...
async function finishExistingSetup(){
 const pin=$('#newPin').value.trim(),pin2=$('#newPin2').value.trim(),name=$('#newName').value.trim();
...
$('#copyCreatedRecovery').onclick=async()=>navigator.clipboard.writeText($('#createdRecovery').value);$('#downloadCreatedRecovery').onclick=()=>downloadText(`FREE-Recovery-${me.shortId}.free-recovery`,$('#createdRecovery').value);
...
$('#changePinBtn').onclick=async()=>{try{const oldPin=$('#oldPin').value.trim(),p1=$('#changePin1').value.trim(),p2=$('#changePin2').value.trim(),kit=await getKV('accountRecoveryKit');if(!kit)throw new Error('Recovery Kit missing');if(p1!==p2||!validPin(p1))throw new Error(currentLang==='vi'?'PIN mới phải gồm 4–6 số và nhập lại phải khớp.':'New PIN must be 4–6 digits and match.');const {secret}=await decryptAccountRecoveryKit(kit,oldPin);const newKit=await buildAccountRecoveryKit(p1,secret);await setKV('accountRecoveryKit',newKit);$('#accountRecoveryKit').value=newKit;$('#oldPin').value=$('#changePin1').value=$('#changePin2').value='';alert(currentLang==='vi'?'Đã đổi PIN. Hãy lưu Recovery Kit mới.':'PIN changed. Save the new Recovery Kit.')}catch(e){alert(e.message)}};
$('#refreshRecovery').onclick=async()=>{const pin=prompt(currentLang==='vi'?'Nhập PIN hiện tại để cập nhật Recovery Kit:':'Enter current PIN to refresh the Recovery Kit:');if(!pin)return;try{const old=await getKV('accountRecoveryKit');const {secret}=await decryptAccountRecoveryKit(old,pin);const kit=await buildAccountRecoveryKit(pin,secret);await setKV('accountRecoveryKit',kit);$('#accountRecoveryKit').value=kit;alert(currentLang==='vi'?'Recovery Kit đã được cập nhật với dữ liệu hiện tại.':'Recovery Kit refreshed with current data.')}catch(e){alert(e.message)}};
$('#copyAccountKit').onclick=async()=>navigator.clipboard.writeText($('#accountRecoveryKit').value);$('#downloadAccountKit').onclick=()=>downloadText(`FREE-Recovery-${me.shortId}.free-recovery`,$('#accountRecoveryKit').value);
