...
async function restoreAccountFromKit(){
 ...
 try{
  ...
  if(progress)progress.textContent=currentLang==='vi'?`Đã khôi phục ${profile?.displayName||me.shortId}. Đang vào FREE…`:`Restored ${profile?.displayName||me.shortId}. Entering FREE…`;
  showApp();
  requestAnimationFrame(()=>{window.scrollTo(0,0);if(progress)progress.textContent=''});
